...
class ModelArguments:
	...
	freeze_token_embed: bool = field(
		default=False,
		metadata={"help": "Freeze the token embedding layer parameters"},
	)
	pretrained_token_embed: Optional[str] = field(
		default=None,
		metadata={
			"help": "Initialize the token embedding layer with pre-trained token embeddings"
		},
	)
...
def read_txt_embeddings(file_name):
		...
		for line in f_in:
			...
			if line:
				if len(line.split(" ")) == 2:
					...
					if vocabulary not in word2id:
						if vocabulary.endswith("@@"):
							vocabulary = vocabulary[:-2]
						else:
							vocabulary = vocabulary + "</w>"

						word2id[vocabulary] = len(word2id)
						id2word[word2id[vocabulary]] = vocabulary
...
def main():
	...
	if model_args.model_name_or_path:
		...
		logger.warning(f"Total parameters in the model ={n_params/2**20:.2f}M params")

		if model_args.freeze_token_embed:
	...
	if model_args.freeze_token_embed:
		assert model_args.pretrained_token_embed is not None

		logger.warning(
			"Reading pre-trained token embeddings from {0}".format(
				model_args.pretrained_token_embed
			)
		)
		embeddings, word2id, id2word = read_txt_embeddings(
			model_args.pretrained_token_embed
		)
		...
		logger.warning(
			"Embedding dimension is {0}".format(
				model.bert.embeddings.word_embeddings.weight.size()
			)
		)

		with torch.no_grad():
			for every_word in word2id:
				if len(tokenizer.encode([every_word], add_special_tokens=False)) != 1:
					logger.error(
						every_word
						+ " is split into multiple sub-words instead of one "
						+ str(tokenizer.encode([every_word], add_special_tokens=False))
					)
					exit()

				pre_trained_embed_index = word2id[every_word]
				bert_embedding_index = tokenizer.encode(
					[every_word], add_special_tokens=False
				)[0]

				if isinstance(model, BertForMaskedLM):
					model.bert.embeddings.word_embeddings.weight[
						bert_embedding_index
					] = torch.FloatTensor(embeddings[pre_trained_embed_index]).to(
	...
	if data_args.line_by_line:
		...
		def tokenize_function_block(examples):
			return tokenizer(
				examples[text_column_name], return_special_tokens_mask=True
			)

		with training_args.main_process_first(desc="dataset map tokenization"):
			tokenized_datasets_pre_group = raw_datasets.map(
				tokenize_function_block,
				batched=True,
				num_proc=data_args.preprocessing_num_workers,
		...
		def group_texts_by_block(examples):
		...
		with training_args.main_process_first(desc="grouping texts together"):
			tokenized_datasets = tokenized_datasets_pre_group.map(
				group_texts_by_block,
				batched=True,
				num_proc=data_args.preprocessing_num_workers,
				load_from_cache_file=not data_args.overwrite_cache,
	...
	if training_args.do_eval:
		last_checkpoint = get_last_checkpoint(training_args.output_dir)
		temp_model = AutoModelForMaskedLM.from_pretrained(last_checkpoint)
